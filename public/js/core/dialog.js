...
(function () {
  ...
  function baue({ titel, text, art, wert, platzhalter, okText, abbruchText, gefahr }) {
    ...
    wrap.innerHTML = `
      <div class="dlg-card" role="dialog" aria-modal="true">
        ...
        <div class="dlg-knoepfe">
          ...
          <button type="button" class="dlg-ok${gefahr ? " gefahr" : ""}">${esc(okText || "OK")}</button>
        </div>
      </div>`;
    return wrap;
  }
  ...
  function zeige(opts) {
    ...
    return new Promise((fertig) => {
      ...
      const abbrechen = () => schliessen(opts.art === "eingabe" ? null : false);
      ...
      function aufTaste(e) {
        ...
        else if (e.key === "Enter" && (opts.art !== "eingabe" || document.activeElement === input)) {
      ...
      else wrap.querySelector(".dlg-ok").focus();
    });
  }
